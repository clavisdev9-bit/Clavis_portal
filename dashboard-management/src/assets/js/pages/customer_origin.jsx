...
const CustomerTable = () => {
    ...
    const defaultColumns = [1, 2, 3, 4, 5, 6, 7];

    // locked columns (tidak bisa di-uncheck)
    ...
    const toggleColumn = (index) => {
        // locked column tidak bisa dihide
    ...
    const optionalColumns = columns
        .map(col => col.index)
        .filter(index =>
            !defaultColumns.includes(index) &&
            !lockedColumns.includes(index)
        );

    const allChecked = optionalColumns.every(index =>
        visibleColumns.includes(index)
    );

    const handleCheckAll = () => {
        if (allChecked) {
            // hanya sisakan default + locked
            const remainColumns = [
                ...new Set([...defaultColumns, ...lockedColumns])
            ];

            setVisibleColumns(remainColumns);
        } else {
            // tampilkan semua
