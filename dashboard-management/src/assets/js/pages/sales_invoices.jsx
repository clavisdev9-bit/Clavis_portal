...
function SalesInvoicesCard() {
    ...
    useEffect(() => {
        ...
        axios.get(`${__API_URL__}/sales/total_orders_by_company`, {
            params
        })
        .then(res => {
    ...
    useEffect(() => {
        ...
        // Tambahkan filter
        params.filter_by = selectedFilterBy;
        axios.get(`${__API_URL__}/sales/active_customer`, {
            params
        })
        .then(res => {
    ...
    useEffect(() => {
        ...
        axios.get(`${__API_URL__}/sales/companies`, {
            params
        })
        .then(res => {
    ...
    useEffect(() => {
        ...
        axios.get(`${__API_URL__}/sales/report_mtd`, {
            params,
        })
        .then(res => {
    ...
    useEffect(() => {
        ...
        axios.get(`${__API_URL__}/sales/report_ytd`, {
            params,
        })
        .then(res => {
    ...
    useEffect(() => {
        ...
        axios.get(`${__API_URL__}/sales/get_company_sales_stats`, {
            params,
            signal: controller.signal
        })
        .then(res => {
    ...
    // Effect 1: create/destroy chart — HANYA saat data atau company berubah
    // Effect 1: create/destroy chart — HANYA saat data atau company berubah
    useEffect(() => {
        if (!Array.isArray(deliveryStatus) || !deliveryStatus.length || !chartRef6.current) return;
        const targetEl = chartRef6.current[selectedCompany];
        if (!targetEl) return;

        const statusOrder = ['full', 'partial', 'pending'];
        const statusColors = {
            full: '#1baf7a',
            partial: '#eda100',
            pending: '#e34948',
        ...
        const dataMap = {};
        deliveryStatus.forEach(function (item) {
        ...
        const categories = statusOrder.map(function (s) {
            return s.charAt(0).toUpperCase() + s.slice(1);
        });

        const seriesData = statusOrder.map(function (s) {
            const entry = dataMap[s];
            const val = entry ? entry.percentage : undefined;
            return (val === undefined || val === null || isNaN(val)) ? 0 : val;
        });

        const totalAmounts = statusOrder.map(function (s) {
            const entry = dataMap[s];
            const val = entry ? entry.total_amount : undefined;
            return (val === undefined || val === null || isNaN(val)) ? 0 : val;
        });

        const barColors = statusOrder.map(function (s) {
            return statusColors[s];
        });

        totalAmountsRef6.current[selectedCompany] = totalAmounts;
        ...
        var basicBar = {
            ...
            dataLabels: {
                ...
                formatter: function (val, opts) {
                    ...
                    const amount = totalAmountsRef6.current[selectedCompany][idx];
            ...
            tooltip: {
                y: {
                    formatter: function (val, opts) {
                        const idx = opts.dataPointIndex;
                        const amount = totalAmountsRef6.current[selectedCompany][idx];
                        return 'Persentase: ' + val.toFixed(2) + '%<br/>Total: ' + formatCurrency(amount);
                    },
                },
            },
        };
        ...
        chartInstance6.current[selectedCompany] = new ApexCharts(targetEl, basicBar);
    ...
    }, [deliveryStatus, selectedCompany]);

    // Effect 2: hanya update opsi dataLabels — TIDAK destroy/create chart
    useEffect(() => {
        const chart = chartInstance6.current[selectedCompany];
        if (!chart) return;
    ...
    useEffect(() => {
        ...
        axios.get(`${__API_URL__}/sales/get_top_customers`, {
            params,
        })
        .then(res => {
    ...
    useEffect(() => {
        ...
        axios.get(`${__API_URL__}/sales/order_fullfilment`, {
            params,
        })
        .then(res => {
    ...
    useEffect(() => {
        ...
        axios.get(`${__API_URL__}/sales/top_category`, {
            params,
        })
        .then(res => {
    ...
    useEffect(() => {
        ...
        // Jika data kosong, destroy chart yang ada (jika ada) dan jangan render apapun
    ...
    useEffect(() => {
        ...
        axios.get(`${__API_URL__}/sales/top_brands`, {
            params,
        })
        .then(res => {
                        ...
                        {companies.map((company) => (
                            ...
                            >
                                ...
                                <div className="grid grid-cols-12 gap-4 mt-2">
                                    ...
                                    <div className="col-span-12 xl:col-span-5 min-h-0">
                                        <div className="p-6 card h-full flex flex-col min-h-0">
                                            ...
                                            {/* <div ref={(el) => {
                                                if (el) {
                                                    chartRef3Map.current[company.id] = el;
                                                }
                                            }} /> */}
                                        </div>
                                    </div>
                                ...
                                {/* <div class="grid grid-cols-12 mt-2 gap-4">
                                    ...
                                    <div className="col-span-12 md:col-span-12 xl:col-span-2 grid grid-cols-2 md:grid-cols-2 xl:grid-rows-2 xl:grid-cols-1 gap-4">
                                        ...
                                        {reportYtd && reportYtd.length > 0 && (() => {
                                            ...
                                            return (
                                                <div className="card col-span-1">
                                                    <div className="flex flex-col gap-2 mb-0 text-base dark:text-gray-300">
                                                        <span><i className="fa-solid fa-money-bill-1-wave"></i> &nbsp;Current Year Sales</span>
                                                        <p className="text-muted text-sm mb-1">({data.label_tahun_ini})</p>
                                                    </div>
                                                    <h4 className="flex items-center gap-4 text-2xl font-semibold text-slate-800 dark:text-slate-100">
                                                        {formatCurrency(totalTahunIni)}
                                                        <span
                                                    ...
                                                    <SourceDataLink
                                                        url={buildSourceUrl("/sales/report_yoy", { company_id: selectedCompany })}
                                                    />
                                                </div>
                                            );
                                        })()}
                                            ...
                                            return (
                                                <div className="card col-span-1">
                                                    <div className="flex flex-col gap-2 mb-0 text-base dark:text-gray-300">
                                                        ...
                                                        <p className="text-muted text-sm mb-1">({data.label_bulan_ini})</p>
                                                    </div>
                                                    <h4 className="flex items-center gap-4 text-2xl font-semibold text-slate-800 dark:text-slate-100">
                                                        {formatCurrency(totalBulanIni)}
                                                        <span
                                    ...
                                    <div class="col-span-12 md:col-span-12 xl:col-span-5 card">
                                        <div ref={(el) => {
                                            if (el) {
                                                chartRef4.current[company.id] = el;
                                            }
                                        }} />
                                    </div>
                                </div> */}
...
const root = ReactDOM.createRoot(document.getElementById('sales_invoices'));
